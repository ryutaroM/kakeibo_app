...
fn input_register_type() -> u8 {
    ...
    io::stdin()
        ...
        .expect("登録の入力に失敗しました");
    let register_type: u8 = register_type.trim().parse().expect("登録種別は数値で入力");
    services::validate::InputValidater::validate_register_type(register_type);
    register_type
}
...
fn input_category_type(register_type: u8) -> u8 {
    ...
    services::validate::InputValidater::validate_category_type(register_type, category_type);
    category_type
}
...
fn input_price() -> u32 {
    ...
    price.trim().parse().expect("金額は数値で")
}

fn input_date() -> NaiveDate {
    println!("{}", "日付を入力(yyyy-mm-dd");
    let mut date = String::new();
    io::stdin().read_line(&mut date).unwrap();
    NaiveDate::from_str(&date).expect("日付はyyyy-mm-ddで入力してください")
}
